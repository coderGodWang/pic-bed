...
rem ---------------------------------------------------------------------------------------------
rem    改进和修复:
rem   2020/9/16 V18版本更新说明：
rem        * 解决部分用户执行脚本后自动反复运行的问题（闪退）。
rem        * 尝试优先使用内网DNS服务进行域名解析。
rem
rem   2020/8/10 V17版本更新说明：
rem        * 增加809错误代码的解决方案3；更正描述句词错误问题。
rem
...
whoami/groups | find "High Mandatory Level" >nul 2>&1
if %errorlevel% EQU 0 (
    cls
    goto Main
) else (
...
IF EXIST %ES001% (
    ...
    <nul set /p=提示：无法连接VPN，请先“保存登录信息”，按任意键将跳转到“保存登录信息”菜单...
    pause >nul
    goto SaveLogin
)
goto :eof
...
if "x%VpnName%" == "x" (
    <nul set /p=提示：VPN连接名称为空，无法自动连接，请先“保存登录信息”，按任意键将跳转到“保存登录信息”菜单...
    pause >nul
    goto SaveLogin
)
...
if "x%VpnUser%" == "x" (
    <nul set /p=提示：VPN用户名为空，无法自动连接，请先“保存登录信息”，按任意键将跳转到“保存登录信息”菜单...
    pause >nul
    goto SaveLogin
)
if "x%VpnUser%" == "xUSER" (
...
if "x%VpnPassword%" == "x" (
    <nul set /p=提示：VPN密码为空，无法自动连接，请先“保存登录信息”，按任意键将跳转到“保存登录信息”菜单...
    pause >nul
    goto SaveLogin
)
...
if %errorlevel% EQU 0 (
    ...
    nslookup eip.teamshub.com 172.16.9.60 1>nul 2>nul
    nslookup sso.teamshub.com 172.16.9.60 1>nul 2>nul
    goto :eof
) else (
...
if "%errorid%" == "809" (
    ...
    rem ################################################################################
    echo 参考解决方案1：
    echo     尝试（禁用/启用）重启外网网卡后再进行VPN连接。
    echo.
    echo 参考解决方案2：
    echo     如果你的主机刚开机或从休眠中恢复，请等待后台系统服务启动完成，再连接VPN。
    echo.
    echo 参考解决方案3：
    echo    尝试切换另部手机热点或家用wifi（重启手机或路由器）后再进行VPN连接。来源：guoxh
    echo.
    goto OtherHelp
)
